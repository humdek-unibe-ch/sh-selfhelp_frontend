...
- **CMS Tailwind CSS (`css` / `css_mobile`).** Section `global_fields.css` and `global_fields.css_mobile` are applied at render time — Tailwind cannot see DB content during build, so classes must be registered explicitly.
  - **Web (`css`):** `src/globals.css` is the source of truth. It uses Tailwind v4 `@source "../examples/**/*.json"` to scan importable bundles/section examples (`examples/cms-in-cms/*.bundle.json`, `examples/sections/*.json`, `examples/pages/*.bundle.json`, etc.) and `@source inline(...)` brace patterns for author-typed classes from the CMS dropdown/free-text field. When adding or changing Tailwind classes in example bundles, run `npm run audit:cms-css` — it fails if web tokens are not covered by either the examples `@source` or the inline safelist patterns. If authors need a new utility family (gradients, rings, backdrop blur, …), extend the inline patterns in `globals.css`; do not rely on hard-coded class names in components alone.
  - **Mobile (`css_mobile`):** uses the Uniwind allow-list from `@selfhelp/shared` (`classifyClass`). Only `px-md`, `py-lg`, `gap-sm`, etc. — not raw Tailwind. Validate with `node scripts/validate-ai-examples.mjs` / `classifyClassString`.
  - **Dropdown catalogue:** `useCssClasses` loads `GET /frontend/css-classes` from the backend asset `public/assets/tailwind-classes.json` (generated by `sh-selfhelp_backend/scripts/generate-css-classes.js`). After expanding the catalogue, run `node scripts/generate-css-classes.js` in the **backend** repo, restart Symfony if needed, then hard-refresh the admin CMS (React Query caches `css-classes` with a long stale time). Keep the JSON list aligned with what `globals.css` can generate.
  - **Do not** reintroduce `src/utils/css-safelist.ts` or `tailwind.config.ts` `safelist` — Tailwind v4 moved safelisting into CSS (`@source` / `@source inline`).
