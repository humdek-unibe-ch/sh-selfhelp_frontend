...
- Test utilities live under `src/test-utils/` (`renderWithProviders`, `msw/createHandlers`, `playwright/loginAs`). Snapshots in `src/test-utils/snapshots/` are committed backend responses; update them only intentionally (rule 18).
