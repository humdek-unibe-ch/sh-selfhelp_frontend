...
- **CMS Tailwind CSS (`css` / `css_mobile`).** Section `global_fields.css` and `global_fields.css_mobile` are applied at render time — Tailwind cannot see DB content during build, so classes must be registered explicitly.
  - **Web (`css`):** `src/globals.css` registers two Tailwind v4 sources: `src/config/cms-tailwind-classes.json`, an exact class-only snapshot of the backend dropdown catalogue, and `@source "../examples/**/*.json"` for importable bundles/section examples (`examples/cms-in-cms/*.bundle.json`, `examples/sections/*.json`, `examples/pages/*.bundle.json`, etc.). Do not add broad `@source inline(...)` Cartesian products: they can generate hundreds of thousands of candidates and exhaust production builds. When the backend catalogue changes, run `npm run sync:cms-css` here, then `npm run audit:cms-css`. Free-typed classes are supported only when present in the catalogue, an importable example, or statically visible frontend source.
  ...
  - **Dropdown catalogue:** `useCssClasses` loads `GET /frontend/css-classes` from the backend asset `public/assets/tailwind-classes.json` (generated by `sh-selfhelp_backend/scripts/generate-css-classes.js`). After changing it, run the backend generator, run `npm run sync:cms-css` in this repo to refresh `src/config/cms-tailwind-classes.json`, restart Symfony if needed, then hard-refresh the admin CMS (React Query caches `css-classes` with a long stale time).
