 ...
 * riding along in the cached section payload. As of v2 the section hook delegates
 * to the unified context-aware picker (`context: 'section'`); these guards pin
 * that contract: the hook stays disabled without a section id, and otherwise
 * returns the backend's token => label map verbatim for the editor's mention
 * picker.
 */
...
describe('useSectionDataVariables — on-demand fresh picker fetch', () => {
    ...
    it('returns the backend token => label map for the picker', async () => {
        ...
        // Delegates to the unified endpoint with the `section` context.
        expect(getVariables).toHaveBeenCalledWith('section', 230);
