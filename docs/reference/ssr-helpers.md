...
| `getNavigationSSR(languageId)`            | `INavigationPayload`                                             | Menu trees from `GET /navigation`; seeds the SSR header/footer chrome. Replaces the legacy `getMenuPagesSSR` helper. |
| `getFrontendPagesSSR(languageId)`         | `IPageItem[]`                                                    | Full ACL-filtered page tree for one language (admin / legacy paths). Seeds `['frontend-pages', languageId]`. |
...
| Helper                                                                                  | Lives in                                  | Job                                                                                                                                                                                                                  |
|-----------------------------------------------------------------------------------------|-------------------------------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `transformNavigationPages(rawPages)`                                                    | `src/utils/navigation.utils.ts`           | Legacy page-tree transform used by admin page lists; public chrome now consumes `GET /navigation` instead.                                                     |
| `selectMenuPages` / `selectFooterPages` / `selectProfilePages`                          | `src/utils/navigation.utils.ts`           | Legacy predicates on the page tree; retained for admin previews until fully retired.                                                                                                    |
